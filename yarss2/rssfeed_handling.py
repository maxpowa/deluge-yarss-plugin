...
def fetch_subscription_torrents(config, rssfeed_key, subscription_key=None):
    ...
    log.info("Update handler executed on RSS Feed '%s (%s)' update interval %d." %
...
def handle_ttl(rssfeed_data, rssfeed_parsed, fetch_data):
    ...
    if rssfeed_parsed.has_key("ttl"):
        ...
        try:
            ...
            if ttl > 0 and ttl < 524160:
                ...
                log.warn("TTL value is invalid: %d" % ttl)
        except ValueError:
...
def fetch_subscription(subscription_data, rssfeed_data, fetch_data):
    ...
    if fetch_data["rssfeed_items"] is None:
        ...
        if rssfeed_parsed.has_key("items"):
            ...
            handle_ttl(rssfeed_data, rssfeed_parsed, fetch_data)
        else:
...
class RSSFeedTimer(object):
    ...
    def __init__(self, config):
        ...
        self.rssfeed_timers = {}
        self.add_torrent_func = torrent_handling.add_torrents # To make it possible to disable adding torrents in testing
    ...
    def set_timer(self, key, interval):
        ...
        try:
            ...
            log.error("Failed to convert interval '%s' to int!" % str(interval))
