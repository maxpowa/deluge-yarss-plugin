...
def download_torrent_file(torrent_url, cookie_header):
    ...
    try:
        ...
        log.error("Failed to download torrent url: %s. Exception: %s" % (torrent_url, str(e)))
        return None
    # Get the info to see if any exceptions are raised
    try:
        torrent_info = lt.torrent_info(lt.bdecode(filedump))
    except Exception, e:
        log.error("Unable to decode torrent file!: %s\nURL:%s" % (str(e), torrent_url))
        return None
    return filedump
...
def add_torrent(torrent_url, cookie_header, subscription_data=None):
    ...
    if torrent_url.startswith("magnet:"):
        log.info("Adding magnet: '%s'" % torrent_url)
        ...
        log.info("Adding torrent: '%s'" % torrent_url)
